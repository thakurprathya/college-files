//to implement numerical intergration by simpson 3/8 rule
#include<iostream>
#include<math.h>
using namespace std;
...
int main(){
 ...
 for(int i=1; i<=subInterval-1; i++){
      k = lower + i*stepSize;
      if(i%3==0){ integrationForm += (2 * (1/(1+pow(k,2)))); }
      else{ integrationForm += (3 * (1/(1+pow(k,2)))); }
 }
 integrationForm *= (stepSize*3/8);
